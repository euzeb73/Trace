...
class Selecteur():
    def __init__(self, terrain: Terrain) -> None:
        ...
        self.slider_largeur = Slider(
            ax=axeslider, label='Largeur', valmin=5, valmax=5000, valinit=200, orientation='vertical')
        ...
        # les rectangles utile pour 1er appel à connect
        self.rectangles: list[Rectgle] = []
        self.connect()
        plt.show()
    ...
    def change_largeur(self, val):
        ...
        if len(self.largeurs) > 0:
            ...
            self.rectangles[-1] = Rectgle(self.points[-2], self.points[-1],
                                          self.largeurs[-1], self.terrain.cellsize)  # actualiser le rectangle
    ...
    def click(self, event):
        ...
        if event.inaxes != self.axes or not self.terrain.isinTerrain(x, y):
            return
        # on ajoute le point cliqué à la liste des points
        ...
        if len(self.points) > 1:  # si on a au moins 2 points il faut ajouter un rectangle
            ...
            self.rectangles.append(Rectgle(
                self.points[-2], self.points[-1], self.largeurs[-1], self.terrain.cellsize))
        self.draw()
    ...
    def valider(self, event):
        ...
        if not self.calculated:
            self.tracetoplot = Traces(self.terrain, self.points)
